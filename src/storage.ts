...
import type { DiffRecord, FileSnapshot } from "./types";

export class DiffHistoryDB extends Dexie {
  diffs!: Table<DiffRecord, string>;
  snapshots!: Table<FileSnapshot, string>;

  constructor() {
    super("obsidian-diff-history");
...
export class StorageManager {
  private db: DiffHistoryDB;

  constructor(db?: DiffHistoryDB) {
    this.db = db ?? new DiffHistoryDB();
  }
  ...
  async deleteByFile(filePath: string): Promise<number> {
    const diffCount = await this.db.diffs
      .where("filePath")
      .equals(filePath)
      .delete();
  ...
  async getDatabase(): Promise<DiffHistoryDB> {
    return this.db;
  }
